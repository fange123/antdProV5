...
const Index: React.FC<IProps> = (props) => {
    ...
    (values: any) => {
      return {
        url: `${location.pathname.replace(
          '/basic-list',
          '',
        )}?page=${pageQuery}&sort=create_time&order=${orders}`,
        params: values,
    ...
    return (
      <Row>
        ...
        <Col xs={24} sm={12} className={styles.table_tool_bar}>
          ...
          />
          <Space>{ActionsBuild(init?.data?.layout?.tableToolBar, actionHandler)}</Space>
        </Col>
      </Row>
    );
  };
  ...
  const batchToolBar = () => {
    ...
    return (
      selectedRowKeys.length > 0 && (
        <Space>{ActionsBuild(init?.data?.layout?.batchToolBar, actionHandler)}</Space>
      )
    );
  ...
  return (
    <PageContainer>
      ...
      <AntdModal modalVisible={modalVisible} hideModal={hideModal} modalUrl={modalUrl} />
